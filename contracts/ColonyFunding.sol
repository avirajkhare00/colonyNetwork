...
contract ColonyFunding is ColonyStorage, DSMath {
  ...
  function getTaskPayout(uint256 _id, uint256 _role, address _token) public view returns (uint256) {
    ...
    return (uint8(task.roles[_role].rating) >= uint8(TaskRatings.Satisfactory)) ? task.payouts[_role][_token] : 0;
  }
  ...
  {
    Task storage task = tasks[_id];
    require(task.roles[_role].user == msg.sender, "colony-claim-payout-denied-invalid-user");

    if (uint8(task.roles[_role].rating) < uint8(TaskRatings.Satisfactory))
      return;
  ...
  auth
  {
    ...
    // Only allow sending from/to created pots
    require(_fromPot <= potCount, "colony-funding-nonexistent-pot");
    require(_toPot <= potCount, "colony-funding-nonexistent-pot");
    ...
    // unless the task was cancelled or everyone has been paid out.
    if (fromTaskId > 0) {
      Task storage task = tasks[fromTaskId];
      if (task.finalized)
        updateRemainingPayouts(fromTaskId, _token);
      require(task.cancelled || task.totalPayouts[_token] == 0 || isTaskDraft(fromTaskId), "colony-funding-task-bad-state");
    }
  ...
  function remainingPayouts(uint256 _id, address _token) public view returns(uint256) {
    return tasks[_id].totalPayouts[_token];
  }
